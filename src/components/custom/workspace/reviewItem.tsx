    ...
    return (
        <Tooltip>
            <TooltipTrigger asChild>
                <div
                    className="flex items-center w-full p-2 rounded-lg hover:bg-popover/70 cursor-pointer"
                    onClick={() => openTab({ type: "chat", title: review.title, chatId: review.id })}
                >
            ...
            <TooltipContent side="right" className="relative">
                ...
                <p className="text-sm text-muted-foreground pl-4 max-w-120 break-words">{review.query.length > 100 ? review.query.slice(0, 100) + "..." : review.query}</p>
