...
export default function SignupPage() {
    ...
    const signUp = async () => {
        ...
        if (error || profileError) {
            setDialogType('error');
            setDialogMessage(error?.message || profileError?.message || "An error occurred");
            setDialogOpen(true);
        } else {
                ...
                <DialogContent showCloseButton={false} className="sm:max-w-md">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-bold">
                            {dialogType === 'success' ? 'Signup Successful 🎉' : 'Signup Failed 😕'}
                        </DialogTitle>
                        <DialogDescription className="text-md mt-2">
                            {dialogType === 'success'
                                ? 'You have successfully signed up. Where would you like to go next?'
                                : dialogMessage}
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="flex flex-row justify-end gap-2 mt-4">
                        {dialogType === 'success' ? (
                            ...
                            <Button onClick={() => setDialogOpen(false)} className="w-full sm:w-auto">Close</Button>
                        )}
                    </DialogFooter>
                </DialogContent>
            </Dialog>
